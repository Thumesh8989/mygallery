...
interface ChartDataPoint {
  time: string;
  value: number;
  // Add other properties if your data points have them
}
...
export default function Chart({data, yLabel, jumpToTimecode}: ChartProps) {
  ...
  const yScale = scaleLinear()
    .domain([min(vals) ?? 0, max(vals) ?? 0]) // Provide default for min/max if vals is empty
    .nice()
    .range([yMax, margin]);

  const yTicks = yScale.ticks(Math.max(1, Math.floor(height / 70)));

  const lineGen = line<ChartDataPoint>() // Specify data type for line generator
    .x((d) => xScale(d.time) ?? 0) // Provide default if xScale(d.time) is undefined
    .y((d) => yScale(d.value));
    ...
    <svg className="lineChart" ref={chartRef}>
      ...
      <g
        ...
        {data.map(({time}, i) => {
          ...
          if (xPos === undefined) return null; // Skip rendering if xPos is undefined
          return (
            <text
              key={i}
              x={xPos}
        ...
        {data.map(({time, value}, i) => {
          ...
          return (
            <g key={i} className="dataPoint"
              ...
              onClick={() => jumpToTimecode(timeToSecs(time))}
              onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') jumpToTimecode(timeToSecs(time))}}
              aria-label={`Data point at time ${time}, value ${value}. Click to jump.`}
            >
              <circle cx={xPos} cy={yScale(value)} r={4} aria-hidden="true" />
              <text x={xPos} y={yScale(value) - 12} aria-hidden="true">
                {value}
              </text>
            </g>
          );
